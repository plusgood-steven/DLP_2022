...
def imshowSeq(val_tensors,pred_tensors, title, path):
    pred_tensors = pred_tensors.cpu().clone().squeeze(0)
    val_tensors = val_tensors.cpu().clone().squeeze(0)
    plt.rcParams["figure.figsize"] = [7.00, 1.50]
    plt.rcParams["figure.autolayout"] = True
    plt.title(title)
    for a in range(2):
        tensors = val_tensors if a == 0 else pred_tensors
        for i in range(1,len(tensors) + 1):
            plt.subplot(2,len(tensors), a * len(tensors) + i)
            plt.xticks([])
            plt.yticks([])
            imshow(tensors[i - 1])
    plt.savefig(path)
